...
export default function DatasetsPage() {
  ...
  return (
      <Box p={4} bg="gray.50" minH="calc(100vh - 64px)">
        ...
        {/* Adjust Flex to create a 50-50 split */}
        <Flex direction={isMobile ? "column" : "row"} height="100%">
          {/* 50% width for Dataset List */}
          <Box width={isMobile ? "100%" : "50%"} bg="white" borderRadius="md" boxShadow="sm" mb={isMobile ? 4 : 0} mr={isMobile ? 0 : 6}>
            <Suspense fallback={<Spinner />}>
              <DatasetList refreshTrigger={refreshTrigger} />
            </Suspense>
          </Box>

          {/* 50% width for Dataset Details */}
          <Box flex={1} bg="white" borderRadius="md" boxShadow="sm" p={6} width={isMobile ? "100%" : "50%"}>
            <Suspense fallback={<Spinner />}>
              {datasetName ? (
